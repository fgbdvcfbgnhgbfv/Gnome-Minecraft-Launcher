...
# FIX: os.chdir() убран — глобальная смена рабочей директории ломала
# относительные пути в других частях приложения (например, game_dir в subprocess).
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _on_close_request(self, window):
        if self._game_running:
            self.show_toast("Дождитесь завершения игры перед выходом")
    ...
    def on_ms_login_clicked(self, btn):
        ...
        def do_login():
            ...
            GLib.idle_add(btn.set_label, "Войти через Microsoft")
    ...
    def init_main_page(self):
        ...
        # Выбираем иконку >= 24 px из fav/ чтобы GTK не растягивал маленький
        # файл вверх — именно апскейл даёт размытость
        _root = os.path.normpath(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
        )
        fav_dir = os.path.join(_root, "fav")
        icon_path = os.path.join(_root, "icon.png")  # fallback
        if os.path.isdir(fav_dir):
            _sizes = []
            for _f in os.listdir(fav_dir):
                if _f.startswith("fav@") and _f.endswith(".png"):
                    try:
                        _sizes.append((int(_f[4:-4]), os.path.join(fav_dir, _f)))
                    except ValueError:
                        pass
            if _sizes:
                _sizes.sort()
                icon_path = next(
                    (p for s, p in _sizes if s >= 24),
                    _sizes[-1][1]   # если всё меньше 24 — берём самый большой
                )
    ...
    def append_console(self, text):
        def _do():
            ...
            # GTK4: TextView → Viewport → ScrolledWindow
            # get_parent() once gives Viewport (no get_vadjustment),
            # get_parent() twice gives ScrolledWindow which has it.
            viewport = self.console_text.get_parent()
            if viewport:
                sw = viewport.get_parent()
                if sw and hasattr(sw, "get_vadjustment"):
    ...
    def init_modrinth_page(self):
        ...
        content.append(self.modrinth_listbox)

        clamp = Adw.Clamp(maximum_size=620)
        clamp.set_child(content)
        # Single ScrolledWindow wraps everything — no nested scrollers
        scrolled = Gtk.ScrolledWindow()
        scrolled.set_vexpand(True)
        scrolled.set_child(clamp)
        box.append(scrolled)
    ...
    def _get_current_mc_version(self):
        ...
        if not item:
            return None
        # Strip OptiFine suffix so callers always get a clean MC version string
        return item.get_string().replace(" + OptiFine", "")
    ...
    def refresh_instances(self):
        ...
        all_names = ["Vanilla (Default)"] + dirs
        # Rebuild the whole model to avoid off-by-one errors with splice
        self.instance_model.splice(0, self.instance_model.get_n_items(), all_names)
